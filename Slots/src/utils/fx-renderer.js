...
export function preloadFx(game) {
    ...
    for (var j = 0; j < PiecSettings.bonus.length; j++) {
        if (PiecSettings.bonus[j].winAnimations != null) {
            for (var i = 0; i < PiecSettings.bonus[j].winAnimations.length; i++) {
                switch (PiecSettings.bonus[j].winAnimations[i]) {
                    case 'coin-line-burst-01':
                        CoinLineBurst01.preload(game);
                        break;
                    case 'coin-line-burst-02':
...
export function playFx(game, layer, currentReelArray, currentSpin) {
    if (currentReelArray[currentSpin].winAnimations != null) {
        for (var i = 0; i < currentReelArray[currentSpin].winAnimations.length; i++) {
            switch (currentReelArray[currentSpin].winAnimations[i]) {
                case 'coin-line-burst-01':
